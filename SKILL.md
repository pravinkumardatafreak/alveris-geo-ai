...
### Worked example — "cluster these points"

> Before clustering: plot the points on their own, with the axes visible, and
> tell me two things — what range the x and y axes span, and whether the points
> land where you expect on a map.
>
> The reason is that most clustering algorithms take a distance threshold, and
> that threshold is in whatever units the geometry is in. If these are lon/lat
> degrees, `eps=500` is 500 degrees and every point collapses into one cluster;
> `eps=0.005` is roughly 500 m at the equator but roughly 300 m at 55° north.
> The axis range tells us which we are dealing with, and whether the clustering
> needs a projected CRS first.
>
> If you can also say roughly how many clusters you expect and at what
> separation, I can sanity-check the output against that.

### Worked example — "join these points to census tracts"

> Before the join: plot both layers on one map, points over tracts, axes on.
> Do the points land inside the tract polygons?
>
> A spatial join with mismatched CRSs does not raise — it returns zero rows, or
> a handful of accidental matches, and looks like a data problem rather than a
> projection problem. Seeing them overlap first rules that out in ten seconds.
>
> Also tell me the row count of each layer. After the join I want to check the
> output against both: fewer rows than points means unmatched points were
> dropped, more rows than points means tracts overlap and some points matched
> twice, which will double-count anything we aggregate afterwards.

### Worked example — "buffer these and count what falls inside"

> Two things first. Plot the features with the axes visible and tell me the
> axis range, and tell me what real-world distance the buffer is meant to be.
>
> Buffering is the operation where unit errors are most invisible: the code
> runs, the polygons look like polygons, and the counts come out as plausible
> integers. A 500 m buffer applied to degree coordinates produces a shape
> covering most of a hemisphere, and the count that comes back is "all of
> them", which reads as a correct answer to a badly-scoped question.
>
> If the axes are in degrees we reproject to a local projected CRS before
> buffering, and I will want to know the latitude band to pick one.

### Worked example — "these areas look wrong"

> Give me three things: the axis range from a plot of the polygons, the CRS the
> areas were computed in, and one polygon whose real area you know or can
> measure on a basemap.
>
> Area is the measure that fails most quietly, because it fails by a *factor*
> and the distribution keeps its shape. If the numbers are off by roughly
> 12,000,000,000× the geometry is in degrees. If they are off by a factor that
> grows as you move north, the CRS is Web Mercator, which is conformal and not
> equal-area — areas in EPSG:3857 are inflated everywhere except the equator.
> The single hand-checked polygon distinguishes those two from a genuine data
> problem faster than any amount of code reading.
...
Close each diagnosis with one line:

> This is covered in [[LESSON: topic]].

Routing table:

| Diagnosis | Route to |
|---|---|
| Layers in different windows, CRS mismatch | [[LESSON: coordinate reference systems]] |
| `set_crs` used where `to_crs` was meant | [[LESSON: assigning versus transforming a CRS]] |
| Coordinates at (0, 0) or in the wrong hemisphere | [[LESSON: coordinate order and null island]] |
| Buffer or distance in degrees | [[LESSON: units and projected coordinate systems]] |
| Areas wrong by a latitude-dependent factor | [[LESSON: equal-area versus conformal projections]] |
| Small persistent offset at building scale | [[LESSON: datums and ellipsoids]] |
| Raster misaligned by a fraction of a cell | [[LESSON: raster transforms and pixel registration]] |
| Categorical raster with invented class values | [[LESSON: resampling methods]] |
| Spatial join returning zero rows | [[LESSON: spatial predicates and join semantics]] |
| Spatial join inflating row counts | [[LESSON: one-to-many joins and double counting]] |
| Null or invalid geometry propagating downstream | [[LESSON: geometry validity]] |
| Aggregates plausible, individual values wrong | [[LESSON: verification against ground truth]] |
